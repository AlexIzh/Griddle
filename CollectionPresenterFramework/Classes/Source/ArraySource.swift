...
open class ArraySource<Element> : DataSource, ExpressibleByArrayLiteral {
	...
	//MARK: - Life cycle
	convenience required public init(arrayLiteral elements: Element...) {
	...
	public func perform(actions: [ArrayAction<Element>]) {
		...
		for action in actions {
			switch action {
			case let .insert(index, model): self.insert(model, at: index)
			case let .delete(index): self.deleteItem(at: index)
			case let .move(oldIndex, newIndex): self.moveItem(from: oldIndex, to: newIndex)
			case let .replace(index, newModel): self.replaceItem(at: index, with: newModel)
			}
		}
