...
open class TablePresenter<DataSourceModel: DataSource>: Presenter<DataSourceModel>, UITableViewDataSource, UITableViewDelegate {
	...
	open override func dataSourceDidBeginEditing(_ dataSource: DataSourceModel) {
		tableView.beginUpdates()
	}
	open override func dataSourceDidEndEditing(_ dataSource: DataSourceModel) {
        ...
        for cell in map.registrationItems {
			...
			case let .nib(nib):
				if cell.itemType != .row {
					self.tableView.register(nib, forHeaderFooterViewReuseIdentifier: cell.identifier)
				} else {
			...
			case let .viewClass(cellClass):
				if cell.itemType != .row {
					self.tableView.register(cellClass, forHeaderFooterViewReuseIdentifier: cell.identifier)
				} else {
					self.tableView.register(cellClass, forCellReuseIdentifier: cell.identifier)
				}
			}
        }
    }
