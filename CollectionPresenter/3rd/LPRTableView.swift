...
public protocol LPRTableViewDelegate: NSObjectProtocol {
   ...
   @objc optional func tableView(_ tableView: UITableView, didMoveCell fromIndexPath: IndexPath, toIndexPath: IndexPath)
}
...
extension LPRTableView {
   ...
   func gestureForCell() -> UILongPressGestureRecognizer {
      return UILongPressGestureRecognizer(target: self, action: #selector(_longPress(_:)))
   }
      ...
      if gesture.state == .began {
         ...
         UIView.animate(withDuration: 0.3,
                        ...
                        completion: { [unowned self] (_: Bool) in
                           ...
                           if let from = self.initialIndexPath, let toIndexPath = self.currentLocationIndexPath {
                              self.longPressReorderDelegate?.tableView?(self, didMoveCell: from, toIndexPath: toIndexPath)
                           }
   ...
   internal func _scrollTableWithCell(_: CADisplayLink) {
